...
async function getAssetData(asset) {
  ...
  const sends = (sendsRes?.result || []).map(s => ({
    id: s.tx_hash, asset, type: 'transfer', btcPrice: null,
    quantity: s.quantity ?? 1,
    from: shortAddr(s.source), to: shortAddr(s.destination),
  ...
  const orders = (ordersRes?.result || []).map(o => ({
    ...
    quantity: 1, from: shortAddr(o.source), to: null,
  ...
  const lastSale = dispenses.length > 0 ? { price: dispenses[0].btcPrice, timestamp: dispenses[0].timestamp } : null;
  return {
    ...
    dispenserCount: dispensers.length, dispensers, dispenses,
    totalSales: dispenses.length, lastSale, transactions, openseaSales,
